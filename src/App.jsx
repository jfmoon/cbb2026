...
function BracketTeamSlot({ team, isWinner, gameState, scores, gameDate, onClick, pickStatus }) {
  ...
  return (
    ...
    >
      ...
      {/* Pick overlay */}
      {pickStatus === "correct"  && <span style={{ fontSize: 9, fontWeight: 700, color: "#16a34a", flexShrink: 0, marginLeft: 2 }}>✓</span>}
      {pickStatus === "wrong"    && <span style={{ fontSize: 9, fontWeight: 700, color: "#dc2626", flexShrink: 0, marginLeft: 2 }}>✗</span>}
      {pickStatus === "pending"  && <span style={{ fontSize: 9, color: "var(--color-text-tertiary)", flexShrink: 0, marginLeft: 2 }}>·</span>}
    </div>
  );
}

// A single matchup (two team slots with a connector)
function BracketGame({ team1, team2, date, scores, onTeamClick, showRoundLabel, roundLabel, view, roundKey }) {
  const picks = view === "mypicks" && roundKey ? (JOHN_PICKS[roundKey] || {}) : null;
  const getPickStatus = (teamName) => {
    if (!picks || !teamName || teamName === "TBD") return null;
    if (!(teamName in picks)) return "pending";
    const pickedCorrectly = picks[teamName];
    const entries = scores?.[teamName];
    const entry = date ? entries?.find(r => r.date === date) : entries?.[entries?.length - 1];
    const gameOver = entry?.state === "post";
    if (!gameOver) return "pending";
    return pickedCorrectly ? "correct" : "wrong";
  };
  const ps1 = getPickStatus(team1?.name);
  const ps2 = getPickStatus(team2?.name);
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      <BracketTeamSlot team={team1} scores={scores} gameDate={date} onClick={onTeamClick} pickStatus={ps1} />
      <BracketTeamSlot team={team2} scores={scores} gameDate={date} onClick={onTeamClick} pickStatus={ps2} />
    </div>
  );
}

// One region's bracket column
function RegionBracket({ regionData, scores, onTeamClick, view }) {
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start", overflowX: "auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: gameGap }}>
            {r64Games.map((g, i) => (
              <BracketGame key={i} team1={g.team1} team2={g.team2} date={g.date}
                scores={scores} onTeamClick={onTeamClick} view={view} roundKey="R64" />
            ))}
          </div>
        </div>
            ...
            {r32Games.map((g, i) => {
              ...
              return (
                <BracketGame key={i} team1={g.team1} team2={g.team2}
                  date={actualDate} scores={scores} onTeamClick={onTeamClick} view={view} roundKey="R32" />
              );
            })}
          </div>
        </div>
        ...
        <div style={{ ...roundWrapStyle, minWidth: 154 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: gameGap * 4 }}>
            {s16Games.map((g, i) => (
              <BracketGame key={i} team1={g.team1} team2={g.team2}
                date={null} scores={scores} onTeamClick={onTeamClick} view={view} roundKey="S16" />
            ))}
          </div>
        </div>
        ...
        <div style={{ ...roundWrapStyle, minWidth: 148 }}>
          ...
          <div style={{ paddingTop: 16 }}>
            <BracketGame team1={e8Game.team1} team2={e8Game.team2}
              date={null} scores={scores} onTeamClick={onTeamClick} view={view} roundKey="E8" />
          </div>
        </div>
...
function BracketTab({ onTeamClick }) {
  ...
  const [view, setView] = useState("actual"); // "actual" | "mypicks"
    ...
    <div style={{ paddingBottom: "2rem" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: "1rem", flexWrap: "wrap" }}>
        <div style={{
          ...
          {view === "actual"
            ? "Live bracket — winners update automatically. Click any team to view their profile."
            : "Your picks overlaid — ✓ correct, ✗ wrong, · pending. Based on picks recorded before tournament."}
        </div>
        <select value={view} onChange={e => setView(e.target.value)} style={{
          ...
          <option value="actual">Live bracket</option>
          <option value="mypicks">My picks</option>
        </select>
      </div>

      {/* Picks scorecard — only in mypicks mode */}
      {view === "mypicks" && (() => {
        const r64Picks = Object.keys(JOHN_PICKS.R64);
        const r64Correct = r64Picks.filter(k => JOHN_PICKS.R64[k] === true).length;
        const r64Wrong   = r64Picks.filter(k => JOHN_PICKS.R64[k] === false).length;
        const r32Picks = Object.keys(JOHN_PICKS.R32);
        const r32Total = r32Picks.length;
        // R32 correctness = picks that match actual results (from resolveWinner via scores)
        // We can compute this from regionData
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", overflowX: "auto" }}>
        {regionData.map(rd => (
          <RegionBracket key={rd.region} regionData={rd} scores={scores}
            onTeamClick={onTeamClick} view={view} />
        ))}
      </div>
        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
          ...
          <div style={{
            ...
            <BracketGame team1={f4Game1.team1} team2={f4Game1.team2}
              date={f4Game1.date} scores={scores} onTeamClick={onTeamClick} view={view} roundKey="F4" />
          </div>
          ...
          <div style={{
            ...
            <BracketGame team1={f4Game2.team1} team2={f4Game2.team2}
              date={f4Game2.date} scores={scores} onTeamClick={onTeamClick} view={view} roundKey="F4" />
          </div>
          ...
          <div style={{
            ...
            <BracketGame team1={ncgGame.team1} team2={ncgGame.team2}
              date={ncgGame.date} scores={scores} onTeamClick={onTeamClick} view={view} roundKey="NCG" />
          </div>
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        if (filterRound === "R64") return <div style={{ marginBottom:"1.5rem" }}><KenPomR64Compare S={S}/></div>;
