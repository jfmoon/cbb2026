...
function OddsChip({name}){
  const o=ODDS[name];
  if(!o||!o.ok)return<span style={{fontSize:10,color:"var(--color-text-tertiary)",fontStyle:"italic"}}>Lines TBD</span>;
  const sprd=o.s?`${parseFloat(o.s)>0?"+":""}${o.s}`:null;
  const ml=formatML(o.ml);
  return(
    <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",padding:"5px 8px",background:"var(--color-background-secondary)",borderRadius:6,marginBottom:6}}>
      {sprd&&<span style={{fontSize:11,fontWeight:700,color:parseFloat(o.s)<0?"#1a6b3a":"var(--color-text-secondary)"}}>{sprd}</span>}
      {ml&&<span style={{fontSize:10,color:"var(--color-text-secondary)"}}>{ml.display}<span style={{fontSize:9,color:"var(--color-text-tertiary)"}}> ({ml.prob}%)</span></span>}
      {o.ou&&<span style={{fontSize:10,color:"var(--color-text-tertiary)"}}>O/U {o.ou}</span>}
    </div>
  );
}
...
function MatchupCard({ matchup, onTeamClick }) {
  ...
  const sprd1 = o1?.ok && o1.s ? `${parseFloat(o1.s)>0?"+":""}${o1.s}` : null;
  const sprd2 = o2?.ok && o2.s ? `${parseFloat(o2.s)>0?"+":""}${o2.s}` : null;
    ...
    <div style={{maxWidth:720}}>
      ...
      <Section title="Pace">
        ...
        {Object.entries(PACE_DEF).map(([label, desc]) => (
          <Card key={label} accent={PC[label]}>
